...
function Collapsible({ open, children }) {
  ...
  return (
    <div ref={ref}
      style={{ maxHeight: 0, opacity: 0, overflow: 'hidden', transition: 'max-height 0.4s cubic-bezier(0.4,0,0.2,1), opacity 0.3s ease' }}>
      {children}
    </div>
  )
}
...
export default function VotingGuide({ dark, completed, onComplete }) {
  ...
  const pct = Math.round((completed.length / guideSteps.length) * 100)

  return (
    <section id="guide" className="scroll-mt-20" aria-labelledby="guide-h">
      <SectionHeader tag="Voting Guide" title="Step-by-step voting guide"
        subtitle="Click each step to expand. Check off steps as you complete them." dark={dark} />

      {/* Progress bar */}
      <div className="flex items-center gap-4 mb-6">
        <div className={`flex-1 h-2 rounded-full overflow-hidden ${dark ? 'bg-slate-800' : 'bg-slate-200'}`}>
          <div
            className="h-full rounded-full bg-gradient-to-r from-indigo-500 via-violet-500 to-pink-500 transition-all duration-700 ease-out"
            style={{ width: `${pct}%` }} />
        </div>
        <span className={`text-[0.8rem] font-bold whitespace-nowrap tabular-nums ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
          {completed.length}/{guideSteps.length} done
        </span>
        {pct === 100 && (
          ...
          return (
            <div key={i}
              className={`rounded-2xl border overflow-hidden transition-all duration-300
                ${isDone
                  ? dark ? 'border-emerald-800/40 bg-gray-900' : 'border-emerald-200/60 bg-white'
                  : dark ? 'border-violet-900/20 bg-gray-900' : 'border-indigo-100/60 bg-white'
                }
                ${isOpen
                  ? 'shadow-card-hover scale-[1.005]'
                  : 'shadow-card hover:shadow-card-hover hover:scale-[1.002]'
                }`}>
              ...
              <button
                ...
                <div className={`w-11 h-11 rounded-full flex items-center justify-center text-[0.95rem] font-black flex-shrink-0 transition-all duration-300
                  ${isDone
                    ? 'bg-gradient-to-br from-emerald-500 to-teal-500 text-white shadow-[0_4px_12px_rgba(16,185,129,0.3)]'
                    : isOpen
                      ? 'bg-gradient-to-br from-indigo-500 to-violet-600 text-white shadow-glow scale-110'
                ...
                <div className="flex-1 min-w-0">
                  <div className={`font-extrabold text-[1.02rem] tracking-[-0.025em] ${dark ? 'text-white' : 'text-gray-900'}`}>
                    {step.title}
                  </div>
                ...
                {/* Checkbox (click independently to mark done) */}
                <button
                  onClick={e => { e.stopPropagation(); if (!isDone) onComplete(i) }}
                ...
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[0.85rem] border flex-shrink-0 transition-all duration-300
                  ${isOpen
                    ? dark ? 'bg-indigo-900/60 border-indigo-700 text-indigo-400 rotate-180' : 'bg-indigo-50 border-indigo-200 text-indigo-600 rotate-180'
                    : dark ? 'bg-white/5 border-white/10 text-slate-500' : 'bg-slate-100 border-slate-200 text-slate-400'
                  }`}>
                  ▾
                </div>
              </button>
              ...
              <Collapsible open={isOpen}>
                <div className={`border-t px-6 pb-6 ${dark ? 'border-white/5' : 'border-indigo-50'}`}>
                  <p className={`text-[0.92rem] leading-relaxed mt-5 mb-4 ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
                    {step.body}
                  </p>
                  ...
                  <div className="flex flex-col gap-2.5">
                    {step.items.map((item, j) => (
                      <div key={j}
                        className={`flex gap-3 items-start rounded-xl p-3.5 border transition-all duration-200
                          ${dark
                            ? 'bg-white/5 border-white/5 hover:border-indigo-700/40 hover:bg-indigo-950/30'
                        ...
                        <div>
                          <strong className={`block font-extrabold text-[0.88rem] mb-0.5 ${dark ? 'text-white' : 'text-gray-800'}`}>
                            {item.title}
                          </strong>
                  ...
                  <div className="flex gap-3 flex-wrap mt-5">
                    {!isDone ? (
                      <button onClick={() => onComplete(i)}
                        className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-[0.85rem] text-white bg-gradient-to-r from-emerald-500 to-teal-500 shadow-[0_4px_12px_rgba(16,185,129,0.3)] hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(16,185,129,0.4)] transition-all">
